...
fn main() {
    ...
    let mut guard_minute_map = HashMap::new();
    for (guard, shifts) in guards {
        ...
        guard_minute_map.insert(guard,
            match minutes.into_iter()
                   .max_by(|(_, lhs), (__, rhs)| lhs.cmp(&rhs)) {
                       Some(x) => x,
                       None => (0, 0)
                   });
    }

    for (guard, (minute, num_sleeps)) in &guard_minute_map {
        println!("Answer: {} * {} = {}", guard, minute, num_sleeps);
    }

    let (guard, (minute, num_sleeps)) = guard_minute_map
        .into_iter()
        .max_by(|(_, (__, lhs)), (___, (____, rhs))| lhs.cmp(&rhs))
        .unwrap();

    println!("Sleepiest minute {} with number of sleeps {} for guard #{}",
             minute, num_sleeps, guard);
