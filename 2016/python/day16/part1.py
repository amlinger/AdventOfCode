...
"""
def dragon(seed, l):
    return seed[:l] if len(seed) >= l else dragon("{}0{}".format(
            seed, "".join(str((int(c)+1) % 2) for c in reversed(seed))), l)

def checksum(data):
    return data if len(data) % 2 == 1 else checksum("".join(map(str, (
        (sum(map(int, data[i:i+2]))+1) % 2 for i in xrange(0, len(data), 2)))))

if __name__ == '__main__':
    print checksum(dragon("10001001100000001", 272))
