...
(6x1)(1x3)A simply becomes (1x3)A - the (1x3) looks like a marker, but because
            it's within a data section of another marker, it is not treated
            any differently from the A that comes after it. It has a
            decompressed length of 6.
X(8x2)(3x3)ABCY becomes X(3x3)ABC(3x3)ABCY (for a decompressed length of 18),
                because the decompressed data from the (8x2) marker (the
...
def next_index(data, ptr):
    ...
    return (i + _len, data[i:i + _len] * rep)

def explosives(data, ptr=0):
    if (ptr >= len(data)): return ''
    if data[ptr] != '(': return data[ptr] + explosives(data, ptr + 1)
    ptr, filler = next_index(data, ptr + 1)
    return filler + explosives(data, ptr)

assert explosives('ADVENT') == 'ADVENT'
assert explosives('A(1x5)BC') == 'ABBBBBC'
assert explosives('(3x3)XYZ') == 'XYZXYZXYZ'
assert explosives('A(2x2)BCD(2x2)EFG') == 'ABCBCDEFEFG'
assert explosives('(6x1)(1x3)A') == '(1x3)A'
assert explosives('X(8x2)(3x3)ABCY') == 'X(3x3)ABC(3x3)ABCY'

if __name__ == '__main__':
    print len(explosives("".join([x.strip() for x in fileinput.input()])))
