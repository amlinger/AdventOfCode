...
class Bot:
    def __init__(self):
        self.getters, self.values = [], []

    def get_values(self):
        if len(self.values) < 2:
            for getter in self.getters:
                self.values.append(getter())
...
def parse_lines(lines):
    ...
    def set_receiver(_type, index, bound_method):
        if _type == "bot":
            receiver_bot = bots.get(index, Bot())
            receiver_bot.getters.append(bound_method)
            bots[index] = receiver_bot
        elif _type == "output":
            output[index] = bound_method

    for line in lines:
        args = line.split()
        if line.startswith('value'):
            bot = bots.get(args[5], Bot())
            bot.values.append(int(args[1]))
            bots[args[5]] = bot
        elif line.startswith('bot'):
            bot = bots.get(args[1], Bot())
            set_receiver(args[5], args[6], bot.low)
            set_receiver(args[10], args[11], bot.high)
            bots[args[1]] = bot
...
def balance_bots(lines):
    ...
    # Find the correct bot:
    for i, bot in bots.iteritems():
        if bot.low() == 17 and bot.high() == 61:
            print "Bot {}: compares {} with {}".format(i, bot.low(), bot.high())
