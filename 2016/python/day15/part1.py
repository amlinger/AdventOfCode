...
Disc = namedtuple('Disc', ['pos', 'max'])

def digit(reg, line): return int(next(re.finditer(reg, line)).group(0))

def solve(lines):
    discs = [Disc(digit(r'(?<=position )\d+', l), digit(r'(?<=has )\d+', l))
                for l in lines]
    return next(t for t in xrange(0, sys.maxint)
        if all([(d.pos + t + i) % d.max == 0 for i, d in enumerate(discs, 1)]))
