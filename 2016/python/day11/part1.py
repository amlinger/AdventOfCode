...
State = namedtuple('State', ['elevator', 'microchips', 'generators'])

def print_board(state, lookup, levels):
    print "     " + " ".join(l.capitalize()[:4] + "." for l in lookup)
    for floor in reversed(range(levels)):
        print "F{} {}".format(floor + 1, "E" if state.elevator == floor else "."),
        for index, pair in enumerate(zip(state.microchips, state.generators)):
            print "{} {}".format(
                    "M" if pair[0] == floor else ".",
                    "G" if pair[1] == floor else "."
                    ).center(5), 
        print ""

def valid_state(state, _dir):
    def validate_line(s, floor):
        # If there is no generator on the floor, no radiation
        return (all(g != floor for g in s.generators) or
            # Verify that all microchips on the floor are shielded by its generator
            all(s.generators[i] == floor for i, m in enumerate(s.microchips) if m == floor))
    return validate_line(state, state.elevator) and validate_line(state, state.elevator - _dir)


def movables(state, levels):
    def _movables(s, _dir, _items, l):
        if not (0 <= s.elevator + _dir < l): return []
        return filter(lambda s: valid_state(s, _dir), [State(
                s.elevator + _dir, 
                [m + (_dir if i in item[0] else 0) for i, m in enumerate(s.microchips)], 
                [g + (_dir if i in item[1] else 0) for i, g in enumerate(s.generators)] 
            ) for item in _items])

    def _items(s):
        singles = (
            [([m], []) for m, p in enumerate(s.microchips) if p == s.elevator] +
            [([], [g]) for g, p in enumerate(s.generators) if p == s.elevator])
        return (singles + [map(add, *c) for c in combinations(singles, 2)])

    i = _items(state)
    return _movables(state, -1, i, levels) + _movables(state, 1, i, levels)

def serialize(state):
    base = 10
    return (state.elevator
        + sum([(base**(2*i))*m for i, m in enumerate(state.microchips, start=1)])
        + sum([(base**(2*i+1))*g for i, g in enumerate(state.generators, start=1)]))
...
def solve(lines):
    ...
    for floor, line in enumerate(lines):
        for microchip in re.findall(r'\S+(?=\-compatible microchip)', line):
            name_lookup.append(microchip)
            initial_state.microchips.append(floor)
    for floor, line in enumerate(lines):
        for generator in re.findall(r'\S+(?= generator)', line):
            initial_state.generators.insert(name_lookup.index(generator), floor)

    q = deque([([], initial_state)])
    visited = set()

    final_state = serialize(State(levels-1, 
                        [levels-1]*len(initial_state.microchips),
                        [levels-1]*len(initial_state.generators)))

    while len(q) > 0:
        iteration, state = q.popleft()

        serialized_state = serialize(state)
        if serialized_state == final_state:
            for i, s in enumerate(iteration):
                print "\n> Step {}:".format("given initailly" if i == 0 else i)
                print_board(s, name_lookup, levels)
            print "\nReached final state in {} steps:".format(len(iteration))
            print_board(state, name_lookup, levels)
        ...
        q.extend([(iteration + [state], s) for s in movables(state, levels)])
    print "Did not find any solution"

if __name__ == '__main__':
    solve([x.strip() for x in fileinput.input()])
